...
// on FastEdge, setup is called for every new request,
// since server-fastedge.js fires "prepare" on every request
function prep() {
  // allowDynamicBackends(true);

  // This is used within `EnvManager`
  if (!globalThis.fastEdgeEnv) {
    // globalThis.fastlyEnv = new Dictionary("env");
    globalThis.fastEdgeEnv = {
      get: (key) => {
        return getEnv(key);
      },
    };
  ...
  if (!globalThis.log) {
    globalThis.log = new Log({
      ...
      withTimestamps: false, // no need to log ts on FastEdge
    });
  }

  // on FastEdge, the network-context isn't available in global-scope
...
function up() {
  ...
  // nothing else to do on sig-up on FastEdge; fire a sig-go!
  system.pub("go");
}
